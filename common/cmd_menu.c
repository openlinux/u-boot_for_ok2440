...
void main_menu_usage(void)
{
	...
	printf("[7] Download User program uCOS-II or Myapp to SDRAM and Run it\r\n");
...
void menu_shell(void)
{
	...
	while (1)
	{
		...
		switch (c)
		{
			case '1':
			{
				strcpy(cmd_buf, "usbslave 1 0x30008000; nand erase 0x0 0x100000; nand write 0x30008000 0x0 0x100000");
				run_command(cmd_buf, 0);
				break;
			}
			...
			case '2':
			{
				sprintf(cmd_buf, "usbslave 1 0x30008000; nand erase 0x0 0x100000; nand write 0x30008000 0x0 0x100000");
				run_command(cmd_buf, 0);
				break;
			}		//HJ_add end 20090709
#endif
			case '3':
			{
				strcpy(cmd_buf, "usbslave 1 0x30008000; nand erase kernel; nand write.jffs2 0x30000000 kernel $(filesize)");
				run_command(cmd_buf, 0);
#ifdef CONFIG_SURPORT_WINCE
				if (!TOC_Read())
					TOC_Erase();				
...
U_BOOT_CMD(
	menu,	3,	0,	do_menu,
	"menu    - A menu, similar to ViVi download from DNW or NFS\n",
	"	   display a menu, to select the items to do something\n"
	"	   Please select the items as given.\n"
);
